...
class TrajSimi:
    def __init__(self, encoder, path):
        ...
        self.final_embedding_path = path
    ...
    def train(self):
        ...
        trainable_num = sum(p.numel() for p in self.trajsimiregression.parameters() if p.requires_grad)
        print("Total Trainable Parameter Num 1:", trainable_num)

        trainable_num = sum(p.numel() for p in self.encoder.clmodel.encoder_q.parameters() if p.requires_grad)
        print("Total Trainable Parameter Num 2:", trainable_num)
    ...
    def test(self, dataset_type):
        ...
        for i_batch, batch in enumerate(self.trajsimi_dataset_generator_single_batchi(datasets)):
            ...
            embs = self.encoder.interpret(trajs_emb, trajs_emb_p, trajs_len)
            outs = self.trajsimiregression(embs)
            embeddings = outs.data.cpu().numpy()
            print(i_batch, embeddings.shape)
            traj_outs.append(embeddings)
        

        # calculate similarity
        # traj_outs = torch.cat(traj_outs)
        traj_outs = np.vstack(traj_outs)
        time2 = time.time()
        print(traj_outs.shape, "Total_time: ", time2 - time1)
        pickle.dump(traj_outs, open(self.final_embedding_path, "wb"))
    ...
    def load_trajsimi_dataset():
        ...
        trajsimi_simi_dataset_file = '{}_{}_train_distance_matrix_result'.format( \
                                    Config.dataset_file, Config.trajsimi_measure_fn_name)
